...
SETDEFAULTS_FUNC(mod_fastcgi_set_defaults) {
	...
	data_unset *du;
	size_t i = 0;
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi set default fun is called");
    closelog ();
	config_values_t cv[] = {
...
static int fcgi_env_add(void *venv, const char *key, size_t key_len, const char *val, size_t val_len) {
	...
	char *dst;
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi env add is called");
    closelog ();
	if (!key || !val) return -1;
	putenv("LD_PRELOAD=/home/meng/Desktop/syscall.so");
	len = key_len + val_len;
...
static handler_t fcgi_stdin_append(server *srv, handler_ctx *hctx) {
	...
	UNUSED(srv);
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi stdin append is called");
    closelog ();
	/* something to send ? */
...
static handler_t fcgi_create_env(server *srv, handler_ctx *hctx) {
	...
	int request_id;
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi create env is called");
    closelog ();
...
static int fastcgi_get_packet(server *srv, handler_ctx *hctx, fastcgi_response_packet *packet) {
	...
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi get packet is called");
    closelog ();
...
static void fastcgi_get_packet_body(buffer *b, handler_ctx *hctx, fastcgi_response_packet *packet) {
	...
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi get packet body is called");
    closelog ();
	for (chunk *c = hctx->rb->first; c; c = c->next) {
		size_t weHave = buffer_string_length(c->mem) - c->offset;
		if (weHave >= toread) {
...
static handler_t fcgi_recv_parse(server *srv, connection *con, struct http_response_opts_t *opts, buffer *b, size_t n) {
	...
	int fin = 0;
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi recv parse is called");
    closelog ();
	if (0 == n) {
...
int mod_fastcgi_plugin_init(plugin *p) {
	...
	openlog ("cgi",LOG_PID,LOG_USER);
    syslog (LOG_INFO, "fcgi plugin init is called");
    closelog ();
	p->init         = gw_init;
	p->cleanup      = gw_free;
